...
def main(args):
    ...
    for task_id in inference_config:
        try:
            ...
            # Get frame dimensions for ffmpeg pipe
            frame_height, frame_width = frame_list[0].shape[:2]
            
            # Set up ffmpeg pipe for direct frame streaming (no disk I/O)
            ...
            ffmpeg_cmd = [
                ...
                "-r", str(fps),
                "-i", "-",
                "-vcodec", "h264_nvenc", "-cq", "19", "-preset", "p4",
                "-pix_fmt", "yuv420p",
                temp_vid_path
            ]
            print(f"Starting ffmpeg pipe: {' '.join(ffmpeg_cmd)}")
            ...
            # Execute inference and stream frames directly to ffmpeg
            for i, (whisper_batch, latent_batch) in enumerate(tqdm(gen, total=total)):
                try:
                    processed_frames += int(getattr(whisper_batch, "shape", [len(whisper_batch)])[0])
                    print(f"PROGRESS_FRAMES={processed_frames}")
                except Exception:
                    pass
                ...
                # Composite and pipe each frame immediately (no accumulation)
                for res_frame in recon:
                    bbox = coord_list_cycle[frame_index % len(coord_list_cycle)]
                    ori_frame = copy.deepcopy(frame_list_cycle[frame_index % len(frame_list_cycle)])
                    x1, y1, x2, y2 = bbox
                    if args.version == "v15":
                        y2 = y2 + args.extra_margin
                        y2 = min(y2, frame.shape[0])
                    try:
                        ...
                        # Write original frame if resize fails (same behavior as original)
                        ffmpeg_proc.stdin.write(ori_frame.tobytes())
                        frame_index += 1
                        continue
                    ...
                    # Pipe raw pixels directly to ffmpeg (no disk write)
            ...
            print(f"Generation + encoding time: {gen_end_time - gen_start_time:.3f} seconds")
            ...
            if is_cuda:
                ...
                print(f"Peak VRAM (PyTorch max allocated): {peak_mem_mb:.2f} MiB")
            overall_end_time = time.time()
            print(f"Total script wall time (main): {overall_end_time - overall_start_time:.3f} seconds")
